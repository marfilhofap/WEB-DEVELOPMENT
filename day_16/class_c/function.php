...
function insert_utilijador($id_estudante, $password)
{
    global $kon;
    $sql = "INSERT INTO t_utilijador(id_estudante, password) VALUES('$id_estudante', '$password')";
    $dados = $kon->prepare($sql);
    $dados->execute();
}

function edit_utilijador($id, $id_estudante, $password)
{
    global $kon;
    $sql = "UPDATE t_utilijador SET id_estudante='$id_estudante', password='$password' WHERE id_utilijador='$id'";
    $dados = $kon->prepare($sql);
    $dados->execute();
}
